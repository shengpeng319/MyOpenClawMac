...
## 🤖 Multi-Agent 协作模式 (Agent-to-Agent)

### 核心原则

**复杂任务 → 引入 projectmanager 负责任务拆解和协调**

### Agent 角色定义

| Agent | 角色 | 职责 |
|-------|------|------|
| **main** | 总控 | 接收用户任务，调度其他 agent |
| **projectmanager** | 项目经理 | 任务拆解、计划制定、多 agent 协调 |
| **coder** | 程序员 | 代码开发、技术实现 |
| **researcher** | 研究员 | 信息收集、调研、分析 |
...
main (总控)
    ↓ 判定复杂度
    ├─ 简单任务 → main 直接处理
    └─ 复杂任务 → 引入 projectmanager
              ↓
         projectmanager: 任务拆解 + 分配
              ↓
         ├─ → coder: 编程任务
         ├─ → researcher: 调研任务
         └─ → 其他: 相应任务
...
main → projectmanager: "请拆解这个任务并制定计划"
