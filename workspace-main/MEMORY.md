...
## 「小老鼠奶酪屋」儿童教育游戏 (2026-04-01)

**项目**：儿童借位减法游戏
**架构**：5 subagent 流水线
**功能**：错误诊断引擎 + 速度训练 + 可视化方块联动

**教训**：复杂游戏/应用开发应使用 subagent 流水线分解任务

---

## Q1 金融研报收官 (2026-04-01)

**成果**：NVDA +5.6%，Q1 收官报告已生成发送

---

## AQI=165 早安预警 (2026-04-01)

**发现**：早安问候包含 AQI=165 空气质量预警，说明天气数据源已支持 PM2.5 预警

---

## 跨 Agent 文件路径传递铁律（已合并，2026-04-01）

**核心问题**：financialadvisor 说"已保存 results/report_20260331.md"，researcher 全 workspace 搜索找不到

**原因**：报告在 `workspace-financialadvisor/results/`，不是 main workspace。每个 agent workspace 是独立文件系统，文件不会自动共享。

**铁律**：跨 agent 通知**必须发文件绝对路径**，不依赖对方 workspace 搜索
...
## 进化报告 Cron 超时问题已自行修复 (2026-04-03)

**问题**：昨日（4/2）22:00 进化报告报 error，302秒后 Request timed out
**解决**：今日（4/3）22:00 自动恢复正常，无人为干预
**可能原因**：一次性服务抖动 / token 消耗波动

*Updated: 2026-04-03*

---

## Subagent 任务拆分规范 (2026-04-04)

**问题**：financialadvisor 超时（单个任务消耗过多 token）
**解决**：长任务需在 subagent 层面拆分，避免单次任务超过 5 分钟或 token 上限
**教训**：Cron 报告类任务尤其要注意拆分，复杂分析应拆成多个步骤

*Updated: 2026-04-04*

---

## launchd plist 参数名必须完全匹配命令 (2026-04-05)

**问题**：opencode serve 的 launchd plist 里用了 `--host`，但正确参数是 `--hostname`，导致服务崩溃重启

**教训**：launchd ProgramArguments 里的参数名必须与命令行完全一致，不能使用简称或错误参数名
