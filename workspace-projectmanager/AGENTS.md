...
## Agent Webchat 消息处理规则（2026-03-29 新增）

**收到其他 Agent 的 webchat 回复时，必须立即在飞书上向 Peng 汇报。**

- 不只是心跳回复，任何 Agent 的 webchat 消息都要转发
- 转发格式：「📊 [Agent名] 消息：{内容}」
- 不能因为"只是内部消息"就不转发
...
## 评审审批详细反馈要求（2026-04-01 新增）

**背景**：评审必须给出**足够详细的修改意见**，不能只给分数。
...
| **分数** | 必须有明确评分（如 3.7/10） |
| **逐项明细** | 每个维度单独打分 + 说明 |
| **问题清单** | 具体列出需要修改的内容（不是笼统的"需改进"） |
| **修改建议** | 每个问题都要有具体可操作的修改方向 |
| **优先级** | 标注 P0/P1/P2，便于决定处理顺序 |

### 发送给 Reviewer 的任务 prompt 模板

```
你是 [角色]，请对 [任务名称] 进行评审。

**评审要求**：
1. 评分必须精确到小数点（如 8.5/10）
2. 每个维度单独评分 + 文字说明
3. 列出所有需要修改的具体问题
4. 每个问题给出具体的修改建议
5. 标注优先级（P0=必须改，P1=强烈建议，P2=可选）

**如果忘记看 session**：
请读取以下 session history 了解项目背景：
[这里Annie会附上相关session key]
```

### 忘记背景怎么办

**任何人忘记任务背景时**，立即读取相关 session：
- 项目经理（Annie）会提供相关 session key
- 直接用 `sessions_history(sessionKey, limit=20)` 读取上下文
- 不清楚的地方主动问 Annie，不要瞎猜
